...
async def get_response_from_agent(conversational_agent_executor, user_input:str, user_id:int, record_id:int):
        chat_response = await conversational_agent_executor.ainvoke(
            {"input": user_input},  # 사용자의 면접 대답 전달
            config={"configurable":{"user_id":user_id, "conversation_id":record_id}},
        )
